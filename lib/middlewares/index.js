...
const middlewares = (module.exports = Object.assign({
      ...
      routers.map(
        Item =>
          (Item instanceof Router
            ? Item.routes()
            : Item.prototype instanceof Router ? new Item().routes() : Item)
      )
    )
  },
  bodyparser (options) {
  ...
  throttle (options) {
    return convert(throttle(options))
  },
  serve (...options) {
  ...
  graphql (app, options, path = '/graphql') {
    const server = graphql(options)
