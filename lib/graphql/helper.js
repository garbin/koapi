...
const helper = module.exports = {
  ...
  search (items, options) {
    ...
    const itemsArray = Object.entries(items).map(([searchName, itemConfig]) => ({
      ...
      resolve: itemConfig.resolve || helper.connection.resolve({
        collection: ctx => itemConfig.model.collection(),
        cursor
      }),
    ...
    return {
      ...
      args: relay.connection.args(Object.assign({
  ...
  fetch (items, options = {}) {
    ...
    const itemsArray = Object.entries(items).map(([fetchName, fetchConfig]) => ({
      ...
      resolve: fetchConfig.resolve || (async (root, {id}, ctx, info) => {
        const item = await fetchConfig.model.findById(id, {require: true})
        return item
      }),
