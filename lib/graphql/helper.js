    ...
    return async (root, args, ctx, info) => {
      ...
      const data = await loader.acquire(`${parent.name}-${model.name}`, parents => {
        if (list) {
          ...
          const parentKey = options.parentKey || `${pluralize.singular(model.tableName)}_id`
          return model.forge().query(q => q.whereIn('id', parents.map(parent => parent[parentKey])))
            .fetchAll().then(items => parents.map(parent =>
              items.find(item => item.id === parent[parentKey])
            ))
        }
      }).load(root)
      return data
