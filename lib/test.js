...
class ResourceTester {
  ...
  options (options = {}) {
    let before, assert, data, patch, errorAssert, target
    ...
    return { before, assert, data, patch, errorAssert, target }
  }
  create (options) {
  ...
  item (options) {
    const {before, assert, data, target, errorAssert} = this.options(options)
    const self = this
    const basic = res => {
      expect(res.status).toBe(200)
      return res
    ...
    test(`GET ${this.pathPattern}/:id`, async function () {
      ...
      return await self.req(before(request(self.server).get(`${this.path()}/${origin.id}`))).then(basic).then(res => {
