...
class ResourceTester {
  ...
  create (options) {
    ...
    test(`POST ${this.paths}`, async function () {
      let res
  ...
  update (options) {
    ...
    test(`PATCH ${this.paths}/:id`, async function () {
      let res
      if (this.resources.current) {
        res = this.resources.current
      } else {
      ...
      const ures = self.req(before(request(self.server).patch(`${this.path()}/${origin.body.id}`)))
  ...
  read (options) {
    ...
    test(`GET ${this.paths}`, async function () {
      await (this.resources.current || self.req(before(request(self.server).post(this.path()).send(data))))
      return await self.req(before(request(self.server).get(this.path()))).then(basic).then(res => {
        expect(res.body).toBeInstanceOf(Array)
        return res
      }).then(assert).catch(errorAssert)
    })
    test(`GET ${this.paths}/:id`, async function () {
      const res = this.resources.current || self.req(before(request(self.server).post(this.path()).send(data)))
      const origin = await res
      return await self.req(before(request(self.server).get(`${this.path()}/${origin.body.id}`))).then(basic).then(res => {
  ...
  destroy (options) {
    ...
    test(`DELETE ${this.paths}/:id`, async function () {
      let res
      if (this.resources.current) {
        res = this.resources.current
      } else {
      ...
      const origin = await res
      return await self.req(before(request(self.server).del(`${this.path()}/${origin.body.id}`))).then(basic).then(assert).catch(errorAssert)
    })
    return this
  }
  crud (options) {
