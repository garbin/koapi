...
class ResourceTester {
  ...
  async setup (setup) {
    beforeEach(async () => {
      const utils = {
        request: async (nestedDatas, options = {}) => {
          const { use = req => req } = options
          const middleares = [ use ]
          const items = []
          const responses = []
          for (const index in nestedDatas) {
            const data = nestedDatas[index]
            const response = middleares.reduce(
              (req, middleware) => {
                middleware(req)
                return req
              },
      ...
      setup = isFunction(setup) ? setup : async ({request}) => {
        return await request(!isArray(setup) ? [setup] : setup)
      }
      Object.assign(context, await setup(utils))
    })
  ...
  list (data = null, options = {}) {
    const { assert = assertSuccess(200), assertCatch = assertError(), use = req => req } = options
  ...
  update (data = null, options = {}) {
    const { assert = assertSuccess(202), assertCatch = assertError(), use = req => req, patch } = options
  ...
  destroy (data = null, options = {}) {
    const { assert = assertSuccess(204), assertCatch = assertError(), use = req => req } = options
    test(`PATCH ${this.testPath}/:id`, async () => {
      let item
      if (data) {
        const req = request(this.server).post(
