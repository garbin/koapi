  ...
  list (options) {
    ...
    test(`GET ${this.pathPattern}`, async function () {
      if (data || this.resources.current) {
        await (this.resources.current || self.req(before(request(self.server).post(this.path()).send(data), data)))
      }
      await self.req(before(request(self.server).get(this.path()), data || this.resources.current)).then(basic).then(res => {
