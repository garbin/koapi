...
class ResourceTester {
  ...
  list (options) {
    ...
    test(`GET ${this.pathPattern}`, async function () {
      ...
      await self.req(before(request(self.server).get(this.path()))).then(basic).then(res => {
  ...
  destroy (options) {
    ...
    test(`DELETE ${this.pathPattern}/:id`, async function () {
      ...
      const origin = await res
      await self.req(before(request(self.server).del(`${this.path()}/${origin.body.id}`))).then(basic).then(assert).catch(errorAssert)
    })
    return this
  }
  crud (options) {
