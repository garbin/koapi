...
class ResourceRouter extends Router {
  ...
  constructor (collection, options) {
    ...
    options.singularName = pluralize.singular(options.name)
    options.foreignId = `${options.singularName}_id`
  ...
  create () {
    ...
    this.post(pattern.root, compose(middlewares), async (ctx) => {
      ...
      if (collection(ctx).relatedData) {
        ...
        await ctx.state.resource.save(attributes, Object.assign({}, options.save || {}, ctx.state.options || {}))
      }
  ...
  list () {
    ...
    this.get(pattern.root, convert(paginate(options.pagination)), compose(middlewares),
      async (ctx) => {
        const query = ctx.state.query || collection(ctx).resetQuery()
        if (collection(ctx).relatedData) {
          query.where({
            [collection(ctx).relatedData.key('foreignKey')]: collection(ctx).relatedData.parentId
          })
        }
        if (options.joins) {
        ...
        const resources = await query.fetchPage(Object.assign({}, ctx.pagination, options.fetch))
  ...
  item () {
    ...
    this.get(pattern.item, compose(middlewares), async (ctx) => {
      const query = ctx.state.query || collection(ctx).resetQuery()
      ctx.body = await query.query(q => q.where({[id]: ctx.params[id]}))
  ...
  update () {
    ...
    const update = async (ctx) => {
      ...
      const query = ctx.state.query || collection(ctx).resetQuery()
      ctx.state.resource = await query.query(q => q.where({[id]: ctx.params[id]}))
        .fetchOne({require: true})
      await ctx.state.resource.save(attributes, Object.assign({ patch: true }, options.save || {}, ctx.state.options || {}))
  ...
  destroy () {
    ...
    this.del(pattern.item, compose(middlewares), async (ctx) => {
      const query = ctx.state.query || collection(ctx).resetQuery()
      ctx.state.resource = await query.query(q => q.where({[id]: ctx.params[id]})).fetchOne({require: true})
  ...
  children (...children) {
    const { foreignId, idType, singularName } = this.options
    this.use(
      `${this.pattern.root}/:${foreignId}(${idType})`,
      async(ctx, next) => {
        ctx.state.nested = ctx.state.parents || {}
        ctx.state.nested[singularName] = await this.collection(ctx)
